...
var UserRels = struct {
	RequestorRelationships string
	TargetRelationships    string
}{
	RequestorRelationships: "RequestorRelationships",
	TargetRelationships:    "TargetRelationships",
}

// userR is where relationships are stored.
type userR struct {
	RequestorRelationships RelationshipSlice `boil:"RequestorRelationships" json:"RequestorRelationships" toml:"RequestorRelationships" yaml:"RequestorRelationships"`
	TargetRelationships    RelationshipSlice `boil:"TargetRelationships" json:"TargetRelationships" toml:"TargetRelationships" yaml:"TargetRelationships"`
}
...
func (r *userR) GetRequestorRelationships() RelationshipSlice {
	if r == nil {
		return nil
	}
	return r.RequestorRelationships
}

func (r *userR) GetTargetRelationships() RelationshipSlice {
	if r == nil {
		return nil
	}
	return r.TargetRelationships
}
...
// RequestorRelationships retrieves all the relationship's Relationships with an executor via requestor_id column.
func (o *User) RequestorRelationships(mods ...qm.QueryMod) relationshipQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"relationships\".\"requestor_id\"=?", o.ID),
	)

	return Relationships(queryMods...)
}

// TargetRelationships retrieves all the relationship's Relationships with an executor via target_id column.
func (o *User) TargetRelationships(mods ...qm.QueryMod) relationshipQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"relationships\".\"target_id\"=?", o.ID),
	)

	return Relationships(queryMods...)
}

// LoadRequestorRelationships allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadRequestorRelationships(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`relationships.requestor_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.RequestorRelationships = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.RequestorID {
				local.R.RequestorRelationships = append(local.R.RequestorRelationships, foreign)
				if foreign.R == nil {
					foreign.R = &relationshipR{}
				}
...
// LoadTargetRelationships allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadTargetRelationships(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`relationships`),
		qm.WhereIn(`relationships.target_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.TargetRelationships = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &relationshipR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.TargetID {
				local.R.TargetRelationships = append(local.R.TargetRelationships, foreign)
				if foreign.R == nil {
					foreign.R = &relationshipR{}
				}
...
// AddRequestorRelationships adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.RequestorRelationships.
// Sets related.R.Requestor appropriately.
func (o *User) AddRequestorRelationships(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Relationship) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			RequestorRelationships: related,
		}
	} else {
		o.R.RequestorRelationships = append(o.R.RequestorRelationships, related...)
	}
...
// AddTargetRelationships adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.TargetRelationships.
// Sets related.R.Target appropriately.
func (o *User) AddTargetRelationships(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Relationship) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.TargetID = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			TargetRelationships: related,
		}
	} else {
		o.R.TargetRelationships = append(o.R.TargetRelationships, related...)
	}
